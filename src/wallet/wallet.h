...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zlitspends need to use the cs_spendcache
...
// Possible states for zLIT send
enum ZerocoinSpendStatus {
    ZLIT_SPEND_OKAY = 0,                            // No error
    ZLIT_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZLIT_WALLET_LOCKED = 2,                         // Wallet was locked
    ZLIT_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZLIT_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZLIT_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZLIT_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZLIT_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZLIT_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZLIT_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZLIT_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZLIT_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZLIT_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZLIT_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZLIT_SPENT_USED_ZLIT = 14,                      // Coin has already been spend
    ZLIT_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZLIT_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZLITOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzlitAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzLITTracker> zlitTracker;
    ...
    void setZWallet(CzLITWallet* zwallet)
    {
        zwalletMain = zwallet;
        zlitTracker = std::unique_ptr<CzLITTracker>(new CzLITTracker(strWalletFile));
    }

    CzLITWallet* getZWallet() { return zwalletMain; }
    ...
    /** zLIT reset */
    boost::signals2::signal<void()> NotifyzLITReset;
