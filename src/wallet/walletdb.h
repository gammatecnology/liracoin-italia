...
public:
    ...
    bool WriteZLITSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZLITSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZLITSeed_deprecated(uint256& seed);
    bool EraseZLITSeed();
    bool EraseZLITSeed_deprecated();

    bool WriteZLITCount(const uint32_t& nCount);
    bool ReadZLITCount(uint32_t& nCount);
