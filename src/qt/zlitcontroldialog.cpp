...
#include "ui_zlitcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zlitTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zlitTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zLIT.";
            else if (!mint.isSeedCorrect)
                strReason = "The zLIT seed used to mint this zLIT is not the same as currently hold in the wallet";
            else
