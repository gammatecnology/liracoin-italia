...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZLITModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zlitTracker->HasMintTx(hash))) {
            //zLIT stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZLIT;
...
bool IsZLITType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZLIT:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZLIT || type == TransactionRecord::MNReward) {
