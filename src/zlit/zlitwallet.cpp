...
CzLITWallet::CzLITWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zlit seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZLITSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZLITSeed_deprecated()) {
                    LogPrintf("%s: Updated zLIT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zlit seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zlit wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzLITWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZLITCount(nCountLastUsed);
    else if (!walletdb.ReadZLITCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzLITWallet::Lock()
{
    seedMaster = 0;
}

void CzLITWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzLITWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZLIT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzLITWallet::LoadMintPoolFromDB()
{
...
void CzLITWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzLITWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzLITWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zlitTracker->HasPubcoinHash(pMint.first)) {
...
bool CzLITWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZLIT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zlitTracker which also adds to database
    pwalletMain->zlitTracker->Add(dMint, true);
...
void CzLITWallet::SeedToZLIT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzLITWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzLITWallet::GenerateDeterministicZLIT(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzLITWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzLITWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzLITWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
