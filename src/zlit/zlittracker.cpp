...
CzLITTracker::CzLITTracker(std::string strWalletFile)
{
...
CzLITTracker::~CzLITTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzLITTracker::Init()
{
...
bool CzLITTracker::Archive(CMintMeta& meta)
{
...
bool CzLITTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzLITTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzLITTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzLITTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzLITTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzLITTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzLITTracker::GetSerialHashes()
{
...
CAmount CzLITTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzLITTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzLITTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzLITTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzLITTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzLITTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzLITTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzLITTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzLITTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzLITTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzLITTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzLITWallet* zLITWallet)
{
    bool iszLITWalletInitialized = (NULL != zLITWallet);
    CMintMeta meta;
    ...
    if (! iszLITWalletInitialized)
        zLITWallet = new CzLITWallet(strWalletFile);
    meta.isSeedCorrect = zLITWallet->CheckSeed(dMint);
    if (! iszLITWalletInitialized)
        delete zLITWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzLITTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLITTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzLITTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzLITTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzLITTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzLITTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzLITWallet* zLITWallet = new CzLITWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zLITWallet);
        }
        delete zLITWallet;
        LogPrint("zero", "%s: added %d dzlit from DB\n", __func__, listDeterministicDB.size());
    }
